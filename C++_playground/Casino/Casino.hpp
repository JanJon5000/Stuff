...
class Player{
    ...
    public:
        ...
        ~Player(){
            ...
            while(getline(saveFile, text)){
                ...
                for(int c = 0;c<text.size();c++){ // c (char)
                    if(text[c] == ' '){ // this for basicly creates a list of all the word in the player-line
                        transformedText.push_back(" ");
                    }else{
                        transformedText[transformedText.size()-1] += text[c];
                    }
                }
                if(transformedText.size() >= 2){
        ...
        void addScore(int a){
            score += a; // function adding a Score, what did you expect
        }
